...
class SubGeometryBase {
    ...
    public var faceNormals(get_faceNormals, never):Float32Array;
    public var UVStride(get_UVStride, never):Int;
    public var vertexData(get_vertexData, never):Float32Array;
    public var vertexPositionData(get_vertexPositionData, never):Float32Array;
    ...
    private var _faceTangents:Float32Array;
    ...
    private var _faceNormals:Float32Array;
    private var _faceWeights:Float32Array;
    ...
    public function getIndexBuffer(stage3DProxy:Stage3DProxy):IndexBuffer3D {
        ...
        if (_indicesInvalid[contextIndex]) {
            _indexBuffer[contextIndex].uploadFromInt16Array(_indices, 0, _numIndices);
    ...
    private function updateFaceNormals():Void {
        ...
        if (_faceWeights == null)
            _faceWeights = new Float32Array( Std.int(len / 3) );
	 ...
	 */
    private function updateVertexNormals(target:Float32Array):Float32Array {
        ...
        while (i < lenI) {
            ...
            target[index] = target[index++] + _faceNormals[f1] * weight;
            target[index] = target[index++] + _faceNormals[f2] * weight;
            target[index] = target[index] + _faceNormals[f3] * weight;
            index = normalOffset + _indices[i++] * normalStride;
            target[index] = target[index++] + _faceNormals[f1] * weight;
            target[index] = target[index++] + _faceNormals[f2] * weight;
            target[index] = target[index] + _faceNormals[f3] * weight;
	 ...
	 */
    private function updateVertexTangents(target:Float32Array):Float32Array {
        ...
        while (i < lenI) {
            ...
            target[index] = target[index++] + _faceTangents[f1] * weight;
            target[index] = target[index++] + _faceTangents[f2] * weight;
            target[index] = target[index] + _faceTangents[f3] * weight;
            index = tangentOffset + _indices[i++] * tangentStride;
            target[index] = target[index++] + _faceTangents[f1] * weight;
            target[index] = target[index++] + _faceTangents[f2] * weight;
            target[index] = target[index] + _faceTangents[f3] * weight;
	 ...
	 */
    public function updateIndexData(indices:Int16Array):Void {
        _indices = indices;
	 ...
	 * @private
	 */
    public function get_faceNormals():Float32Array {
        if (_faceNormalsDirty) updateFaceNormals();
        return _faceNormals;
    }
    ...
    public function get_vertexPositionData():Float32Array {
        throw new AbstractMethodError();
        return null;
    }
    ...
    public function scale(scale:Float):Void {
        var vertices:Float32Array = UVData;
    ...
    public function applyTransformation(transform:Matrix3D):Void {
        var vertices:Float32Array = _vertexData;
        var normals:Float32Array = vertexNormalData;
        var tangents:Float32Array = vertexTangentData;
        var posStride:Int = vertexStride;
        var normalStride:Int = vertexNormalStride;
        var tangentStride:Int = vertexTangentStride;
        var posOffset:Int = vertexOffset;
    ...
    private function updateDummyUVs(target:Float32Array):Float32Array {
        _uvsDirty = false;
        var idx:Int;
        var uvIdx:Int;
        var stride:Int = UVStride;
        ...
        if (target == null) target = new Float32Array( len );
